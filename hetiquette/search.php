  ...
  <div class="page">
	...
	<?php
	$q = $_GET['search'];
	$sql= "SELECT COUNT(*) as nbArt FROM s_intensive WHERE `marque` LIKE '%$q%' OR `description` LIKE '%$q%' OR `nom` LIKE '%$q%' OR `prix` LIKE '%$q%'"; // ajouter les criteres where de la recherche
	$stmt = $conn->prepare($sql);
	$stmt->execute();
	$donnes = $stmt->fetch();
	...
	if(isset($_GET['p']) && $_GET['p']>0 && $_GET['p']<=$nbPage){
 	...
 	$ma_requete = "SELECT * FROM `s_intensive` WHERE `marque` LIKE '%$q%' OR `description` LIKE '%$q%' OR `nom` LIKE '%$q%' OR `prix` LIKE '%$q%' LIMIT ".(($cPage-1)*$perPage).",$perPage;";// sélectionne tous les éléments, qui correspondent à la recherche demandé
...
<div class="p">
      ...
      for ($i=1; $i <=$nbPage ; $i++) {
      	...
      	echo "<a href=\"search.php?search=$q&p=$i\">$i </a>";
      }
      }
      ?>
        </div>
